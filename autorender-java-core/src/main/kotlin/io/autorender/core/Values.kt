...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { AutorenderInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw AutorenderInvalidDataException("`$name` is not set")
            is JsonNull -> throw AutorenderInvalidDataException("`$name` is null")
            else -> throw AutorenderInvalidDataException("`$name` is invalid, received $this")
        }
