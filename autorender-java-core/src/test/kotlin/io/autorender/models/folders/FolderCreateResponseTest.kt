...
internal class FolderCreateResponseTest {
    ...
    fun create() {
        val folderCreateResponse =
            FolderCreateResponse.builder().folderNo("folder_no").name("name").build()

        assertThat(folderCreateResponse.folderNo()).contains("folder_no")
        assertThat(folderCreateResponse.name()).contains("name")
    }
